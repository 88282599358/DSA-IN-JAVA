...
If the base case is not met, the recursion continues. First, the sequence method is called recursively with ind incremented by 1, which means we exclude the current element from the subsequence (Remove step).

Then, the current element arr[ind] is added to the subsequence by calling ds.add(arr[ind]).

Next, the sequence method is called recursively again with ind incremented by 1, which means we include the current element in the subsequence (Take step).
